...
class NavigateToGoalPrimitive(PrimitiveBase):
    ...
    def onStart(self):
        ...
        self.tf_listener.waitForTransform('map', 'base_link', rospy.Time(0),
                                          rospy.Duration(5))
    ...
    def _get_pose(self):
        try:
            p, o = self.tf_listener.lookupTransform('map', 'base_link',
                                                    rospy.Time(0))
...
class PickObjectPrimitive(PrimitiveBase):
    def attach_obj(self):
        try:
            ...
            req = AttachRequest()
            req.model_name_1 = "husky"
            req.link_name_1 = "ur_arm_wrist_3_link"
...
#         wall_loc = self.params['Enter wall no:'].value
